...
import { CreditCard, Loader2, CheckCircle, AlertCircle, CreditCardIcon } from "lucide-react"
...
export function PaymentModal({ isOpen, onClose, onSuccess, amount, productName }: PaymentModalProps) {
    ...
    if (preferenceId && typeof window !== "undefined") {
      const loadMercadoPago = async () => {
        try {
          ...
          setPaymentStatus("error")
          setError("Error al inicializar el procesador de pagos. Por favor, intenta nuevamente.")
        }
      }
        ...
        <div className="space-y-6">
          {paymentStatus === "success" ? (
            <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4 text-center">
              ...
              <h3 className="text-lg font-semibold text-green-400">¡Pago completado con éxito!</h3>
              <p className="text-sm text-[#A1A1AA] mt-1">Tu CV está listo para descargar</p>
            </div>
          ) : paymentStatus === "error" ? (
            <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 text-center">
              <AlertCircle className="w-12 h-12 mx-auto text-red-500 mb-2" />
              <h3 className="text-lg font-semibold text-red-400">Error en el pago</h3>
              <p className="text-sm text-[#A1A1AA] mt-1">{error || "Hubo un problema al procesar tu pago"}</p>
              <Button variant="outline" className="mt-4 border-[#3F3F46]" onClick={createPreference}>
                Intentar nuevamente
              </Button>
            </div>
          ) : (
            <>
              <div className="bg-[#2A2A2D] rounded-lg p-4">
                <div className="flex justify-between items-center mb-4">
                  <div>
                    ...
                    <p className="text-sm text-[#A1A1AA]">Optimizado para ATS</p>
                  </div>
                  <Badge className="bg-[#7C3AED] hover:bg-[#7C3AED]">${amount}</Badge>
                </div>
              ...
              {paymentStatus === "loading" && !preferenceId ? (
                <div className="flex flex-col items-center justify-center py-6">
                  ...
                  <p className="text-sm text-[#A1A1AA]">Preparando opciones de pago...</p>
                </div>
              ) : (
